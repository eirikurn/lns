...
async function map_(path, options) {
  ...
  // Check if store has path and we haven't configured conflict resolution.
  const conflict = await fs.exists(target);
  if (conflict && !options.ours && !options.theirs) {
    throw error(`lns: ${path}: Already exists in store. Merge and specify --ours or --theirs.`, true);
  }

  // Make sure the parent directory exists in the store
  await fs.mkdirp(pathUtil.dirname(target));

  // Crude opt-in conflict resolution. Delete either ours or theirs.
  if (conflict) {
    if (options.ours) {
...
async function alreadySynced_(source, userPath) {
  ...
  const syncTarget = store.externalToLocal(source);
